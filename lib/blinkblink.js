...
var ws = require ('ws');
var blinkblinkjs = {
    open: function (url, name, features, attribute) {
        var self = this;
                ...
                response.on ('end', function () {
                    ...
                    for (var index in blinks_attribute) {
                        ...
                        if (type === 'page' || type === 'background_page') {
                            if (url && title && debugger_url && ! self.blinks[id]) {
                                self.blinks[id] = new_blink (blink_attribute);
                            }
                            else if (self.blinks[id]) {
                                self.blinks[id].attribute = blink_attribute;
                            }
                        }
                    }
    ...
    function new_blink (attribute) {
        ...
        b.on ('open', function () {
            if (self._sum === 0) {
                ...
                var name = self.attribute.app.name;
                b.send ('Runtime.evaluate', {
                    expression: 'window.name="' + name + '"'
                });
                self.emit (name, b);
                self._sum ++;
                self.length ++;
                ...
                b.send ('Runtime.evaluate', {
                    ...
                    if (! error && response.result) {
                        self.emit (response.result.value, b);
                        self._sum ++;
                        self.length ++;
                    }
                });
        ...
        b.on ('close', function (reason) {
            self.length --;
            delete self.blinks[b.attribute.id];
        });
        return b;
    }
};
